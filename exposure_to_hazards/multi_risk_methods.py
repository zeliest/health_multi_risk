## I do not use all these methods here, these were in my branch in climada but actually it is easier to just have it here
...
def aggregate_events(impact, how='sum', exp=None, by="date"):
    ...
    if by == "date":
        ...
        if len(np.unique(impact_upscaled_dates.event_name)) == len(np.unique(arr_to_aggr)):
            event_name = np.unique(impact.event_name)
        else:
